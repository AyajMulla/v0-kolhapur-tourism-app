...
for (const [id, coords] of Object.entries(coordsDict)) {
    // Check if coordinates already injected for this id
    // we injected it below id, so let's just do a basic replace
    const targetString = `id: "${id}",`;
    const replacementString = `id: "${id}",\n        coordinates: ${coords},`;
    
    if (content.includes(targetString) && !content.includes(replacementString)) {
        content = content.replace(targetString, replacementString);
        modified = true;
        console.log("Injected coords for", id);
    }
}
...
if (modified) {
    ...
    console.log("No new injections were made.");
}
