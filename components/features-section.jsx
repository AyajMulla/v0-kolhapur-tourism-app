      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <Card
              key={index}
              className="group hover:shadow-xl transition-all duration-300 border-0 bg-white/80 backdrop-blur-sm hover:-translate-y-2 luxury-card"
            >
              <CardContent className="p-6">
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1">
                    <h3 className={`text-xl font-semibold text-gray-800 mb-2 group-hover:text-orange-600 transition-colors ${language !== 'en' ? 'font-devanagari' : ''}`}>
                      {feature.title}
                    </h3>
                    <p className={`text-gray-600 leading-relaxed ${language !== 'en' ? 'font-devanagari text-sm' : ''}`}>
                      {feature.description}
                    </p>
                  </div>
                </div>
